...
- (instancetype)initWithFileURL:(NSURL *)URL {
    if (self = [super init]) {
        ...
        self.attributes = [RTFileInfo attributesWithFileURL:URL];
        
        if ([self.attributes.fileType isEqualToString:NSFileTypeDirectory]) {
            self.type = MLBFileTypeDirectory;
            self.filesCount = [RTFileInfo contentCountOfDirectoryAtURL:URL];
        } else {
            self.extension = URL.pathExtension;
            self.type = [RTFileInfo fileTypeWithExtension:self.extension];
            self.filesCount = 0;
        }
    }
...
//按照时间排序 by liman
+ (NSMutableArray<RTFileInfo *> *)sortedMLBFileInfoArray:(NSMutableArray<RTFileInfo *> *)array
{
    return [[[array copy] sortedArrayUsingComparator:^NSComparisonResult(RTFileInfo  *_Nonnull obj1, RTFileInfo  *_Nonnull obj2) {
    ...
    if (isExists && isDir) {
        ...
        if (!error) {
            for (NSString *name in contents) {
                RTFileInfo *fileInfo = [[RTFileInfo alloc] initWithFileURL:[URL URLByAppendingPathComponent:name]];
